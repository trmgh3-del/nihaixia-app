...
r = filterFormulaSafety(all, {}, { caseType: '心肺症状' }, [])
assert(r.formulas.length === 0, '心肺症状应先排除急症后再谈方剂')
r = filterFormulaSafety(all, {}, { age: 8 }, [])
assert(r.formulas.length === 0, '儿童应由医师确认方剂与剂量')
r = filterFormulaSafety(all, {}, {}, ['胸痛/胸闷'])
assert(r.formulas.length === 0 && r.blocked.length === all.length, '红旗症状应过滤全部方剂')
console.log('PASS: formula safety cases 7')
