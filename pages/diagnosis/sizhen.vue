    ...
    <view v-if="step === 4" class="tab-body fade-in">
      <view class="report card">
        ...
        <view class="report-meta">
          <text>规则版本：{{ result.kbVersion || '本地规则' }}</text><text>待审核规则：{{ result.kbReviewPending || 0 }}</text><text>规则命中：{{ result.kbCoverage }}%</text><text>参考置信度：{{ result.kbConfidence || '不足' }}</text><text>采集完整度：{{ result.completeness }}%</text>
...
export default {
  ...
  methods: {
    ...
    viewReport(item) {
      ...
      store.sizhenReport = item
      uni.navigateTo({ url: '/pages/diagnosis/report' })
    },
    removeReport(i) {
